...
export function PokeCard({ name = "", id = -1 }: Props) {
    ...
    return <Link href={`/${name}?${queryParams}`}>
        <article 
            className="p-2 bg-linear-to-b from-red-600 to-red-800 rounded-lg grid justify-center 
                        place-items-center lg:grid-cols-[1fr_50px] w-36 xl:w-52 shadow-lg outline-white 
                        shadow-black hover:scale-110 hover:outline-2 transition-all duration-200">
